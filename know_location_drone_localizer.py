...
from util import random_vector_of_length
...
class KnownLocationDroneLocalizer():
    def __init__(self,
        particle_count: int,
        weight_calculator: WeightCalculator,
        particle_randomize_percent: float,
        drone_move_noise: float,
        drone_last_known_noise: int,
        initial_location_offset: int):

        self.particle_count = particle_count
        self.particle_randomize_percent = particle_randomize_percent
        self.weight_calculator = weight_calculator
        self.drone_move_noise = drone_move_noise
        self.drone_last_known_noise = drone_last_known_noise
        self.initial_location_offset = initial_location_offset
    ...
    def _generate_initial_particles(self, drone: DroneProvider, city_map: MapProvider):
        drone_position = drone.get_position()
        location_offset = random_vector_of_length(self.initial_location_offset)

        offset_position = drone_position + location_offset

        last_known_particles = [(offset_position.x, offset_position.y) for _ in range(self.particle_count)]
        return self._move_particles(city_map, last_known_particles, 0, 0, self.drone_last_known_noise)

    def fly_drone_route(self, drone: DroneProvider, city_map: MapProvider) -> list[StepResult]:
        particles = self._generate_initial_particles(drone, city_map)
        ...
        while drone.has_step():
            ...
            particles = self._move_particles(city_map, resampled_particles, moved_by.x, moved_by.y, self.drone_move_noise)
